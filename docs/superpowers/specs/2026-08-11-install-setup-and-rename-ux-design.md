...
### 実装で確定した重要点（実機検証で判明）
- **`SetForegroundWindow` 単独では効かない**: 直前に対象がフォアグラウンドを取った状態では OS のフォアグラウンドロックで無視される（実機で naive 版は対象が前面のまま＝復帰失敗を確認）。**`AttachThreadInput` で前面スレッドへ一時アタッチしてから `BringWindowToTop`+`SetForegroundWindow`** することで確実に復帰する（実機で robust 版の復帰成功を確認）。
- **成否に関わらず復帰**: プレビュー/キャプチャは対象前面化の後で失敗し得る（対象が最小化など）。前面化した可能性がある以上、`finally` で必ず `BringToolToForeground()` を呼び、失敗時もツールが隠れたままにしない。
- **管理者対象**: 対象が管理者ウィンドウで SCtool が非管理者だと UIPI で前面操作がブロックされる。ただし選択時に `ConfirmAndElevateForTarget` が昇格を促す既存フローがあり、承諾すれば両者管理者で成立。断った場合は `catch` で握りつぶし前面復帰しないだけに留める（既存の劣化挙動と一貫、クラッシュしない）。
